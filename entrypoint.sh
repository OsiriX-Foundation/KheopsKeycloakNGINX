...
if [[ -f "${LETSENCRYPT_DIR:-/etc/letsencrypt}/live/login-lavim.unige.ch/privkey.pem" ]]; then
    ...
    openssl x509 -req -days 365 -in /usr/share/nginx/certificates/cert_unige.csr -signkey /usr/share/nginx/certificates/privkey_unige.pem -out /usr/share/nginx/certificates/fullchain_unige.pem
